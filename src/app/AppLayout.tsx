...
const MainContent: React.FC<AppLayoutProps> = ({ controller }) => {
  const { view, currentUser, stats, xpToNextLevel, wotd, setGlobalViewWord, setView, lastBackupTime, handleBackup, restoreFromServerAction, triggerLocalRestore, triggerLocalBackup, triggerServerBackup, handleGoogleDriveBackup, restoreFromGoogleDriveAction, handleNavigateToList, sessionWords, sessionType, sessionFocus, updateWord, bulkUpdateWords, handleSessionComplete, gainExperienceAndLevelUp, recalculateXpAndLevelUp, handleRetrySession, deleteWord, bulkDeleteWords, startSession, initialListFilter, setInitialListFilter, forceExpandAdd, setForceExpandAdd, handleUpdateUser, refreshGlobalStats, handleLibraryReset, apiUsage, xpGained, lastMasteryScoreUpdateTimestamp, isWotdComposed, randomizeWotd, handleComposeWithWord, writingContextWord, consumeWritingContext, serverStatus, serverUrl, networkMode, isSwitchingServerMode, handleToggleServerMode, planningAction, consumePlanningAction, targetGameMode, consumeTargetGameMode, targetCourseId, consumeTargetCourseId, startKotobaDueReviewSession, startKotobaNewLearnSession, startKotobaStatusReviewSession, googleDriveAuthState, handleGoogleDriveLogin, handleGoogleDriveLogout } = controller;
  if (!currentUser) return null;
  switch (view) {
    case 'DASHBOARD': return <Dashboard userId={currentUser.id} user={currentUser} totalCount={stats.total} dueCount={stats.due} newCount={stats.new} xpToNextLevel={xpToNextLevel} wotd={wotd} onViewWord={setGlobalViewWord} setView={setView} lastBackupTime={lastBackupTime} onBackup={handleBackup} onRestore={() => {}} restoreFromServerAction={async () => await restoreFromServerAction()} triggerLocalRestore={triggerLocalRestore} onLocalBackup={triggerLocalBackup} onServerBackup={triggerServerBackup} onGoogleDriveBackup={handleGoogleDriveBackup} onGoogleDriveRestore={restoreFromGoogleDriveAction} googleDriveSignedIn={!!googleDriveAuthState} onGoogleDriveLogin={() => { sessionStorage.setItem('vocab_pro_settings_tab', 'SERVER'); setView('SETTINGS'); }} onNavigateToWordList={(filter) => handleNavigateToList(filter, 'vocab')} onNavigateToKotobaList={(filter) => handleNavigateToList(filter, 'kotoba')} isWotdComposed={isWotdComposed} onComposeWotd={handleComposeWithWord} onRandomizeWotd={randomizeWotd} serverStatus={serverStatus} serverUrl={serverUrl} activeServerMode={networkMode === 'outside' ? 'public' : networkMode} isSwitchingServerMode={isSwitchingServerMode} onToggleServerMode={handleToggleServerMode} onAction={(action) => { controller.handleSpecialAction(action); }} onStartDueReview={controller.startDueReviewSession} onStartNewLearn={controller.startNewLearnSession} onStartKotobaDueReview={startKotobaDueReviewSession} onStartKotobaNewLearn={startKotobaNewLearnSession} />;
    ...
    case 'REVIEW': return sessionWords && sessionType ? <ReviewSession user={currentUser} sessionWords={sessionWords} sessionType={sessionType} sessionFocus={sessionFocus} onUpdate={updateWord} onUpdateUser={handleUpdateUser} onBulkUpdate={bulkUpdateWords} onComplete={handleSessionComplete} onRetry={handleRetrySession} /> : null;
    case 'BROWSE': return <StudyItemList user={currentUser} onDelete={async (id) => await deleteWord(id)} onBulkDelete={async (ids) => await bulkDeleteWords(ids)} onUpdate={updateWord} onUpdateUser={handleUpdateUser} onStartSession={(words) => startSession(words, 'custom')} initialFilter={initialListFilter} onInitialFilterApplied={() => setInitialListFilter(null)} forceExpandAdd={forceExpandAdd} onExpandAddConsumed={() => setForceExpandAdd(false)} onNavigate={setView} />;
    ...
    case 'SETTINGS': return <SettingsView user={currentUser} onUpdateUser={handleUpdateUser} onRefresh={refreshGlobalStats} onNuke={handleLibraryReset} apiUsage={apiUsage} googleDriveSignedIn={!!googleDriveAuthState} googleDriveEmail={googleDriveAuthState?.email} onGoogleDriveLogin={handleGoogleDriveLogin} onGoogleDriveLogout={handleGoogleDriveLogout} />;
