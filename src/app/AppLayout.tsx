...
const MainContent: React.FC<AppLayoutProps> = ({ controller }) => {
  ...
  if (!currentUser) return null;
  switch (view) {
    case 'DASHBOARD': return <Dashboard userId={currentUser.id} user={currentUser} totalCount={stats.total} dueCount={stats.due} newCount={stats.new} xpToNextLevel={xpToNextLevel} wotd={wotd} onViewWord={setGlobalViewWord} setView={setView} lastBackupTime={lastBackupTime} onBackup={handleBackup} onRestore={() => {}} restoreFromServerAction={async () => await restoreFromServerAction()} triggerLocalRestore={triggerLocalRestore} onLocalBackup={triggerLocalBackup} onServerBackup={triggerServerBackup} onNavigateToWordList={(filter) => handleNavigateToList(filter, 'vocab')} onNavigateToKotobaList={(filter) => handleNavigateToList(filter, 'kotoba')} isWotdComposed={isWotdComposed} onComposeWotd={handleComposeWithWord} onRandomizeWotd={randomizeWotd} serverStatus={serverStatus} serverUrl={serverUrl} activeServerMode={networkMode === 'outside' ? 'public' : networkMode} isSwitchingServerMode={isSwitchingServerMode} onToggleServerMode={handleToggleServerMode} onAction={(action) => { controller.handleSpecialAction(action); }} onStartDueReview={controller.startDueReviewSession} onStartNewLearn={controller.startNewLearnSession} onStartKotobaDueReview={startKotobaDueReviewSession} onStartKotobaNewLearn={startKotobaNewLearnSession} />;
