...
const MainContent: React.FC<AppLayoutProps> = ({ controller }) => {
  const { view, currentUser, stats, xpToNextLevel, wotd, setGlobalViewWord, setView, lastBackupTime, handleBackup, restoreFromServerAction, triggerLocalRestore, triggerLocalBackup, triggerServerBackup, handleNavigateToList, sessionWords, sessionType, sessionFocus, updateWord, bulkUpdateWords, handleSessionComplete, gainExperienceAndLevelUp, recalculateXpAndLevelUp, handleRetrySession, deleteWord, bulkDeleteWords, startSession, initialListFilter, setInitialListFilter, forceExpandAdd, setForceExpandAdd, handleUpdateUser, refreshGlobalStats, handleLibraryReset, apiUsage, xpGained, lastMasteryScoreUpdateTimestamp, isWotdComposed, randomizeWotd, handleComposeWithWord, writingContextWord, consumeWritingContext, serverStatus, planningAction, consumePlanningAction, targetGameMode, consumeTargetGameMode, targetCourseId, consumeTargetCourseId } = controller;
  if (!currentUser) return null;
  switch (view) {
    case 'DASHBOARD': return <Dashboard userId={currentUser.id} user={currentUser} totalCount={stats.total} dueCount={stats.due} newCount={stats.new} xpToNextLevel={xpToNextLevel} wotd={wotd} onViewWotd={setGlobalViewWord} setView={setView} lastBackupTime={lastBackupTime} onBackup={handleBackup} onRestore={() => {}} restoreFromServerAction={async () => await restoreFromServerAction()} triggerLocalRestore={triggerLocalRestore} onLocalBackup={triggerLocalBackup} onServerBackup={triggerServerBackup} onNavigateToWordList={handleNavigateToList} isWotdComposed={isWotdComposed} onComposeWotd={handleComposeWithWord} onRandomizeWotd={randomizeWotd} serverStatus={serverStatus} onAction={(action) => { controller.handleSpecialAction(action); }} onStartDueReview={controller.startDueReviewSession} onStartNewLearn={controller.startNewLearnSession} />;
