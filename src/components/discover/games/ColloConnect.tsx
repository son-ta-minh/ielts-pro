...
export const ColloConnect: React.FC<Props> = ({ words, onComplete, onExit }) => {
    const [cards, setCards] = useState<Card[]>([]);
    const [selectedCardIds, setSelectedCardIds] = useState<string[]>([]);
    const [score, setScore] = useState(0);

    useEffect(() => {
        const candidates = words.filter(w => w.collocationsArray && w.collocationsArray.length > 0);
        if (candidates.length < 5) {
            alert("Not enough words with collocations found (need at least 5).");
            onExit();
            return;
        }
        ...
        selectedWords.forEach(w => {
            if (!w.collocationsArray) return;
            const validCollocs = w.collocationsArray.filter(c => !c.isIgnored);
            if (validCollocs.length === 0) return;
            const targetColloc = validCollocs[Math.floor(Math.random() * validCollocs.length)];

            const pairId = w.id + Math.random();
            const headword = w.word;
            let tailText = targetColloc.text;
    ...
    return (
        <div className="flex flex-col h-full relative">
            <div className="flex justify-between items-center mb-6 shrink-0">
                ...
                <div className="text-neutral-400 font-bold text-sm uppercase tracking-widest">Collocation Connect</div>
            </div>
                ...
                {cards.length === 0 ? <div className="absolute inset-0 flex items-center justify-center text-neutral-300 font-black text-2xl">Loading Deck...</div> : (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[100px]">
                        {cards.map(card => {
                            ...
                            if (isSelected) cardStyle = "bg-indigo-600 border-indigo-600 text-white scale-105 shadow-xl ring-4 ring-indigo-100";
