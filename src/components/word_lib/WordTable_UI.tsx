...
export const WordTableUI: React.FC<WordTableUIProps> = ({
  ...
  refinedFilter, statusFilter, registerFilter, sourceFilter, isAddExpanded, isFilterMenuOpen, quickAddInput,
    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      {isAddExpanded && (
        <div className="bg-white p-6 rounded-[2rem] border-2 border-neutral-100 shadow-xl space-y-4 animate-in slide-in-from-top-4 duration-300">
          <textarea autoFocus value={quickAddInput} onChange={(e) => setQuickAddInput(e.target.value)} placeholder="e.g. resilient; ubiquitous; mitigate; ..." className="w-full h-20 p-4 bg-neutral-50 border border-neutral-200 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-neutral-900 focus:outline-none resize-none" />
          <div className="flex justify-end"><button onClick={handleBatchAddSubmit} disabled={isAdding || !quickAddInput.trim()} className="px-6 py-3 bg-neutral-900 text-white rounded-xl font-black text-[10px] flex items-center space-x-2 hover:bg-neutral-800 transition-all active:scale-95 disabled:opacity-50 uppercase tracking-widest shadow-lg">{isAdding ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}<span>Add Words</span></button></div>
        </div>
      )}
      ...
      {isAiModalOpen && selectedWordsToRefine.length > 0 && (
        <UniversalAiModal 
            ...
            description={`Generating details for ${selectedWordsToRefine.length} selected words.`}
