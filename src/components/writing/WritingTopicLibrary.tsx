...
interface Props {
  ...
  topics: WritingTopic[];
  history: WritingLog[];
  historyByTopic: Map<string, WritingLog[]>;
  onStartPractice: (topic: WritingTopic) => void;
  onEditTopic: (topic: WritingTopic) => void;
  onCreateTopic: () => void;
...
const WritingTopicLibrary: React.FC<Props> = ({ user, topics, history, historyByTopic, onStartPractice, onEditTopic, onCreateTopic, onDeleteTopic, onDeleteLog, onStartFullTestSetup }) => {
  const [query, setQuery] = useState('');
  const [topicToDelete, setTopicToDelete] = useState<WritingTopic | null>(null);
  const [logToDelete, setLogToDelete] = useState<WritingLog | null>(null);
  ...
  const renderHistoryDetails = (log: WritingLog) => (
    <div className="space-y-3 mt-3 pt-3 border-t border-neutral-100 text-xs leading-relaxed">
        <div>
            <h5 className="font-bold text-neutral-500 mb-1">Task 1 Response</h5>
            <p className="italic text-neutral-600 whitespace-pre-wrap">"{log.task1Response}"</p>
        </div>
        <div>
            <h5 className="font-bold text-neutral-500 mb-1">Task 2 Response</h5>
            <p className="italic text-neutral-600 whitespace-pre-wrap">"{log.task2Response}"</p>
        </div>
        <div>
            <h5 className="font-bold text-neutral-500 mb-1">Feedback</h5>
            <div dangerouslySetInnerHTML={{ __html: log.feedbackHtml }} />
        </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-neutral-900 tracking-tight flex items-center gap-3"><PenLine size={28}/> Writing Studio</h2>
          <p className="text-neutral-500 mt-2 font-medium">Practice full writing tasks and get holistic feedback.</p>
        </div>
      ...
      <ConfirmationModal 
        ...
        message={<>Are you sure you want to delete this writing log from <strong>{logToDelete && new Date(logToDelete.timestamp).toLocaleString()}</strong>? This action cannot be undone.</>}
        confirmText="Yes, Delete"
        isProcessing={false}
        onConfirm={() => {
...
export default WritingTopicLibrary;