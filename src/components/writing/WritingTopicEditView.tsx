...
import { getRefineWritingTopicPrompt } from '../../services/promptService';
import { Loader2, Save, Sparkles, ArrowLeft, PenLine } from 'lucide-react';
...
const WritingTopicEditView: React.FC<Props> = ({ user, topic, onSave, onCancel }) => {
  ...
  const handleGeneratePrompt = (inputs: { request: string }) => {
    const currentTopicState = { ...topic, name, description, task1, task2 };
    ...
    <>
      ...
      {isAiModalOpen && (
        <UniversalAiModal 
          ...
          type="REFINE_UNIT" // Re-using for UI
...
export default WritingTopicEditView;