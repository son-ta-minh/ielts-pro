...
import { Mic, Square, Play, Ear, Eye, EyeOff, ArrowRight, RotateCw, CheckCircle2, AlertTriangle, Volume2, Waves, Eraser, X, List, RefreshCw, Zap, ChevronLeft, PanelLeft, Sparkles, Loader2, Bot } from 'lucide-react';
...
export const MimicPracticeUI: React.FC<MimicPracticeUIProps> = ({
    ...
    queue, currentIndex, onSelect, autoSpeak, onToggleAutoSpeak, onRefresh, isGlobalMode,
    ...
    if (isEmpty) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-4 relative bg-white">
                ...
                <h3 className="text-xl font-black text-neutral-900">Library Empty</h3>
                <p className="text-neutral-500 max-w-xs">No suitable phrases found for practice.</p>
            </div>
        );
    }
    ...
    // Updated layout logic: Use standard h-full to respect parent padding. 
    // Removed 'absolute inset-...' which was causing double spacing.
    const rootClasses = isGlobalMode
      ? 'h-full rounded-[2.5rem] shadow-sm border border-neutral-200'
      : 'h-full';
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    {isActive && <div className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-pulse shrink-0 mt-1.5" />}
                                </div>
                            </button>
                        );
                    })}
                    ...
                    {!targetText ? (
                        ...
                        <>
                            ...
                            <div className="relative w-full max-w-2xl">
                                <div className={`
                                    p-8 rounded-[2.5rem] border-2 text-center transition-all duration-300 min-h-[150px] flex flex-col items-center justify-center relative
                                ...
                                `}>
                                    ...
                                    {/* Reveal Toggle - Only show when sidebar is collapsed */}
                                    {!isSidebarOpen && (
                                        <button 
                                ...
                                {userTranscript && (
                                    <div className="relative group animate-in slide-in-from-bottom-2 w-full max-w-xl text-center">
                                        ...
                                        <div className="relative">
                                            <p className={`text-xl font-medium px-8 leading-relaxed ${
                                                matchStatus === 'match' ? 'text-green-600' :
                                                matchStatus === 'close' ? 'text-amber-600' :
                                                'text-neutral-800'
                                            }`}>
                                                "{userTranscript}"
                                            </p>
                                            
                                            {/* Clear Button - Only show when recording */}
                                            {isRecording && (
                                                <button 
                                                    onClick={onClearTranscript}
                                                    className="absolute -right-2 top-1/2 -translate-y-1/2 p-2 text-neutral-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-all"
                                                    title="Clear Text"
                                                >
                                                    <Eraser size={16} />
                                                </button>
                                ...
                                <div className="w-full max-w-xl bg-white rounded-2xl border border-neutral-200 p-6 animate-in slide-in-from-bottom-2 shadow-sm">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-black uppercase tracking-widest text-neutral-500">AI Feedback</span>
                                        </div>
                                        <span className={`text-lg font-black ${aiAnalysis.score >= 80 ? 'text-green-600' : aiAnalysis.score >= 60 ? 'text-amber-600' : 'text-red-600'}`}>
                            ...
                            <div className="flex items-center gap-8">
                                <button 
                                    ...
                                    className="p-5 rounded-2xl bg-white border border-neutral-200 text-neutral-600 hover:text-indigo-600 hover:border-indigo-200 transition-all shadow-sm active:scale-95"
                                    title="Listen to phrase"
                                >
                                    <Volume2 size={28} className={isRecording ? 'opacity-50' : ''} />
                                </button>

                                <button 
                                    onClick={onToggleRecord}
                                    className={`
                                        w-28 h-28 rounded-full flex items-center justify-center shadow-2xl transition-all transform
                                        ${isRecording 
                                            ? 'bg-red-500 text-white scale-110 ring-8 ring-red-100' 
                                            : 'bg-white border-4 border-neutral-100 text-neutral-900 hover:border-neutral-200 hover:scale-105'
                                        }
                                    `}
                                >
                                ...
                                <button 
                                    ...
                                    className={`p-5 rounded-2xl border transition-all shadow-sm ${
                                        ...
                                        ? 'bg-white border-neutral-200 text-neutral-600 hover:text-indigo-600 hover:border-indigo-200' 
                                        : 'bg-neutral-50 border-neutral-100 text-neutral-300 cursor-not-allowed'
                                    }`}
                                    title="Replay your recording"
                                >
                                    <Play size={24} fill={userAudioUrl && !isRecording ? "currentColor" : "none"} />
                                </button>
                            </div>

                            {/* Analysis & Next Buttons */}
                            <div className="flex gap-3 pt-2">
                                {userAudioUrl && !isRecording && (
                                    <button
                                        onClick={onAnalyze}
                                        disabled={isAnalyzing}
                                        className="px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-wider flex items-center gap-2 bg-white border border-neutral-200 text-neutral-600 hover:bg-neutral-50 hover:text-indigo-600 transition-all active:scale-95 shadow-sm disabled:opacity-50"
                                    >
                                ...
                                <button 
                                    onClick={onNext} 
                                    disabled={!canProceed}
                                    className={`px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center gap-3 transition-all shadow-xl
                                        ${canProceed 
                                            ? 'bg-neutral-900 text-white hover:bg-neutral-800 active:scale-95 hover:shadow-2xl animate-in fade-in' 
