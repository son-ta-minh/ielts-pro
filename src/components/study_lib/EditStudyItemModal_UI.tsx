...
export interface EditStudyItemModalUIProps {
  ...
  setFlag: (flag: 'isIdiom' | 'isPhrasalVerb' | 'isCollocation' | 'isStandardPhrase' | 'isIrregular' | 'isPassive' | 'isFocus' | 'isFreeLesson' | 'isPrep') => void;
                ...
                {activeTab === 'DETAILS' && (
                    <div className="space-y-6 animate-in fade-in duration-300">
                        <div className="space-y-2">
                           ...
                           <div className="flex flex-wrap gap-2 p-3 bg-white border border-neutral-200 rounded-2xl">{[
                             { id: 'isIdiom', label: 'Idiom', icon: Quote },
                             { id: 'isCollocation', label: 'Colloc.', icon: Combine },
                             { id: 'isStandardPhrase', label: 'Phrase', icon: MessageSquare },
                             { id: 'isPhrasalVerb', label: 'Phrasal', icon: Layers },
                             { id: 'isPrep', label: 'Prep.', icon: AtSign },
                             { id: 'isIrregular', label: 'Irregular', icon: RotateCw },
                             { id: 'isFreeLesson', label: 'Lesson', icon: GraduationCap },
                             { id: 'isPassive', label: 'Archive', icon: formData.isPassive ? Archive : Trash2 },
                             { id: 'isFocus', label: 'Focus', icon: Zap }
                           ].map(btn => (
                             <button
                               key={btn.id}
                               type="button"
                               onClick={() => setFlag(btn.id as any)}
                               className={`flex items-center space-x-1.5 py-1.5 px-3 rounded-lg border transition-all font-bold text-[9px] uppercase tracking-wider ${formData[btn.id] ? 'bg-neutral-900 border-neutral-900 text-white shadow-sm' : 'bg-white border-neutral-200 text-neutral-500 hover:border-neutral-300'}`}
                             >
