...
import { Type, Underline, Minus, Plus, BookText, Link, Unlink, HelpCircle, X, ChevronDown, CheckCircle2 } from 'lucide-react';
...
const MAX_DROPDOWN_CANDIDATES = 4; // 1 correct + 3 distractors
...
// --- Sub-component for individual Practice Input with Dropdown Support ---
    ...
    return (
        <span ref={containerRef} className="relative inline-block align-baseline group/input">
            <div className="flex items-center">
                ...
                {/* Small toggle for dropdown if not correct */}
                {!isCorrect && (
                    <button 
...
const HighlightedEssay: React.FC<{ 
    ...
    // Pool of all words in the unit to use as distractors
    const candidatePool = useMemo(() => {
        const pool = Array.from(mapping.keys()).filter((k: string) => k.length > 2);
        return pool;
    }, [mapping]);

    // Choose which indices to blank out and their candidate lists
...
export interface UnitEssayViewUIProps {
    text: string;
    vocabString?: string;
    wordsByText: Map<string, VocabularyItem>;
    highlightColor: HighlightColor;
...
export const UnitEssayViewUI: React.FC<UnitEssayViewUIProps> = ({
