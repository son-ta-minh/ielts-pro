...
const testResultKeyMap: { [key: string]: string } = {
    ...
    'COLLOCATION_CONTEXT_QUIZ': 'ccq',
    'COLLOCATION_MULTICHOICE_QUIZ': 'cmq',
    'IDIOM_CONTEXT_QUIZ': 'icq',
...
function cleanImportedItem(item: any): any {
    ...
    cleaned.libraryType = normalizeLibraryType(cleaned.libraryType);
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = async (ev) => {
            try {
                ...
                const incomingVocabRaw: any[] = Array.isArray(rawJson) ? rawJson : (rawJson.vocabulary || rawJson.vocab || []);
                const incomingKotobaRaw: any[] = Array.isArray(rawJson) ? [] : (rawJson.kotoba || []);
                const incomingItemsRaw: any[] = [
                    ...incomingVocabRaw.map(item => ({ ...item, libraryType: 'vocab' as StudyLibraryType })),
                    ...incomingKotobaRaw.map(item => ({ ...item, libraryType: 'kotoba' as StudyLibraryType }))
                ];
                ...
                const incomingWordFamilyGroups: WordFamilyGroup[] | undefined = Array.isArray(rawJson) ? undefined : (rawJson.wordFamilyGroups || rawJson.wfg);
                ...
                if (scope.vocabulary) {
                    ...
                    const localItemsByWord = new Map(localItems.map(item => [`${normalizeLibraryType(item.libraryType)}:${item.word.toLowerCase().trim()}`, item]));
                    const itemsToSave: StudyItem[] = []; 

                    for (const incoming of incomingItems) {
                        if (!incoming.word) continue;
                        const incomingLibraryType = normalizeLibraryType(incoming.libraryType);
                        const local = localItemsByWord.get(`${incomingLibraryType}:${incoming.word.toLowerCase().trim()}`);
                        if (local) {
                            if ((incoming.updatedAt || 0) > (local.updatedAt || 0)) {
                                itemsToSave.push({ ...local, ...incoming, id: local.id, userId: importedUserId, libraryType: incomingLibraryType, updatedAt: Date.now() } as StudyItem); 
                                mergedCount++;
                            } else skippedCount++;
                        } else {
                            const now = Date.now();
                            itemsToSave.push({ ...incoming, id: incoming.id || `w-${now}-${Math.random()}`, userId: importedUserId, libraryType: incomingLibraryType, createdAt: incoming.createdAt || now, updatedAt: now } as StudyItem);
                            newCount++;
                        }
                    }
            ...
            if (incomingFocusHistory) localStorage.setItem(FOCUS_PERIOD_HISTORY_KEY, JSON.stringify(incomingFocusHistory));
                ...
                if (scope.lesson) {
                    ...
                    if (incomingWordFamilyGroups) await bulkSaveWordFamilyGroups(incomingWordFamilyGroups.map(group => ({ ...group, userId: importedUserId })));
...
export const generateJsonExport = async (userId: string, currentUser: User, scopeInput: any = FULL_SCOPE) => {
    ...
    const [wordsData, unitsData, readingBooksData, logsData, speakingTopicsData, speakingLogsData, nativeSpeakItemsDataRaw, conversationItemsDataRaw, freeTalkItemsDataRaw, writingTopicsData, writingLogsData, compositionsData, irregularVerbsData, wordFamilyGroupsData, lessonsData, listeningItemsData, wordBooksDataRaw, planningGoalsData] = await Promise.all([
        ...
        scope.lesson ? getWordFamilyGroupsByUserId(userId) : [],
    ...
    const dailyStreaks = readDailyStreaks(userId);
     ...
     return {
        ...
        kotoba: wordsData.filter(w => normalizeLibraryType(w.libraryType) === 'kotoba').map(w => _mapToShortKeys(w)),
        u: unitsData,
        pl: logsData,
